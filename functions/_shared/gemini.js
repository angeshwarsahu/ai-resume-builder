...
// Gemini 3.8 Flash is the current stable Flash model and supports the
// legacy GenerateContent endpoint used by this project.
export const GEMINI_URL = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-3.8-flash:generateContent';
...
export async function callGemini(prompt, apiKey, { maxOutputTokens = 1200 } = {}) {
  ...
  try {
    res = await fetch(`${GEMINI_URL}?key=${encodeURIComponent(apiKey)}`, {
      ...
      body: JSON.stringify({
        ...
        // Gemini 3.8 Flash does not accept the old temperature/top_p/top_k
        // sampling parameters, so only send the supported output limit.
